  ...
  try {
    ...
    if (!apiKey) {
      return jsonResponse(
        {
          ok: false,
          error: 'Missing OPENAI_API_KEY. Please add it in Cloudflare Pages project settings.',
        },
        500
      );
    }
    ...
    const report = await generateReport({ brief, websiteText, apiKey, env: context.env });
...
async function fetchWebsiteText(url) {
  ...
  try {
    const response = await fetch(url, {
      ...
      headers: {
        'User-Agent': 'AdGEO-AI-Diagnostic-Bot/0.1',
      },
    });
...
async function generateReport({ brief, websiteText, apiKey, env }) {
  ...
  return parseJsonReport(outputText);
}
...
function buildPrompt(brief, websiteText) {
  return `
你是一个“AI海外增长投放专家系统”，服务对象是独立站、海外游戏、App、SaaS、跨境电商和出海企业。

你必须基于客户提交的网站和需求，直接产出可交付的增长报告，而不是解释你会怎么做。

重要合规边界：
- 可以提供广告政策风险检查、拒审原因分析、落地页合规建议、账户结构建议、申诉文案辅助。
- 不得提供绕过平台风控、买卖黑户、伪装主体、规避审核、欺诈性开户等建议。
- 如果客户行业可能涉及博彩、金融、成人、加密、医疗、保健品、真钱游戏或高风险游戏机制，必须在合规风险里提示需要牌照、地区限制和平台政策确认。
  ...
  "videoScripts": [
    {
      ...
      "duration": "20-30秒",
      "hook": "前三秒钩子",
      "scenes": ["0-3秒画面", "4-8秒画面", "9-15秒画面", "16-25秒画面", "26-30秒画面"],
      "voiceover": "完整口播文案",
